	...
	var c = {
		...
		options: {
			...
			toolbar: [["style", ["style"]], ["font", ["bold", "italic", "underline", "clear"]],
				...
				["height", ["height"]], ["table", ["table"]], ["insert", ["link", "picture", "video"]],
		...
		this.stylePara = function (b, c) {
			...
			var e = a(f.isText(c.sc) ? c.sc.parentNode : c.sc), g = ["font-family", "font-size", "text-align",
				"list-style-type", "line-height"], h = b(e, g) || {};
		...
		}, g = ["bold", "italic", "underline", "strikethrough", "justifyLeft", "justifyCenter", "justifyRight",
			"justifyFull", "insertOrderedList", "insertUnorderedList", "indent", "outdent", "formatBlock",
			"removeFormat", "backColor", "foreColor", "insertHorizontalRule",
			"fontName"], i = 0, m = g.length; m > i; i++)this[g[i]] = function (a) {
			return function (b, c) {
		...
		var d = new m, e = new o, g = new p, j = new q, k = new r, l = function (b) {
			...
			var b = l(a.currentTarget || a.target), c = d.currentStyle(a.target);
			c && (e.update(b.toolbar(), c), g.update(b.popover(), c), j.update(b.handle(), c))
		}, v = function (a) {
			var b = l(a.currentTarget || a.target);
			g.hide(b.popover()), j.hide(b.handle())
		}, w = function (b) {
			if (f.isControlSizing(b.target)) {
				var c = l(b.target), e = c.handle(), h = c.popover(), i = c.editable(), k = c.editor(), m = e.find(".note-control-selection").data("target"), n = a(m), o = n.offset(), p = a(document).scrollTop();
				k.on("mousemove", function (a) {
					d.resizeTo({
						x: a.clientX - o.left,
						y: a.clientY - (o.top - p)
					}, n, !a.shiftKey), j.update(e, {image: m}), g.update(h, {image: m})
				}).one("mouseup", function () {
					k.off("mousemove")
				}), n.data("ratio") || n.data("ratio", n.height() / n.width()), d.recordUndo(i), b.stopPropagation(), b.preventDefault()
			}
		}, x = function (b) {
			...
			if (h.length > 0) {
				var i, j, m, n = h.attr("data-event"), o = h.attr("data-value"), p = l(g.target), q = p.editor(), r = p.toolbar(), t = p.dialog(), v = p.editable(), w = p.codable(), x = q.data("options");
				if (-1 !== a.inArray(n, ["resize", "floatMe", "removeMedia"])) {
					var y = p.handle(), z = y.find(".note-control-selection");
					m = a(z.data("target"))
				}
				if (d[n] && (v.trigger("focus"), d[n](v, o, m)), -1 !== a.inArray(n, ["backColor",
						"foreColor"]))e.updateRecentColor(h[0], n, o); else if ("showLinkDialog" === n) {
					v.focus();
					var A = d.getLinkInfo();
					d.saveRange(v), k.showLinkDialog(v, t, A).then(function (a, b) {
						d.restoreRange(v), d.createLink(v, a, b)
					})
				} else if ("showImageDialog" === n)v.focus(), k.showImageDialog(v, t).then(function (a) {
					"string" == typeof a ? (d.restoreRange(v), d.insertImage(v, a)) : s(v, a)
				}); else if ("showVideoDialog" === n) {
					v.focus();
					var B = d.getVideoInfo();
					d.saveRange(v), k.showVideoDialog(v, t, B).then(function (a) {
						d.restoreRange(v), d.insertVideo(v, a)
					})
				} else if ("showHelpDialog" === n)k.showHelpDialog(v, t); else if ("fullscreen" === n) {
					var C = a("html, body"), D = function (a) {
						q.css("width", a.w), v.css("height", a.h), w.css("height", a.h), w.data("cmEditor") && w.data("cmEditor").setSize(null, a.h)
					};
					q.toggleClass("fullscreen");
					var E = q.hasClass("fullscreen");
					E ? (v.data("orgHeight", v.css("height")), a(window).on("resize", function () {
						D({w: a(window).width(), h: a(window).height() - r.outerHeight()})
					...
					if (F) {
						if (w.val(v.html()), w.height(v.height()), e.deactivate(r), w.focus(), c.bCodeMirror) {
							j = b.fromTextArea(w[0], a.extend({mode: "text/html", lineNumbers: !0}, x.codemirror));
							var G = q.data("options").codemirror.tern || !1;
							G && (i = new b.TernServer(G), j.ternServer = i, j.on("cursorActivity", function (a) {
								i.updateArgHints(a)
							})), j.setSize(null, v.outerHeight()), j.autoFormatRange && j.autoFormatRange({
								line: 0,
								ch: 0
							}, {line: j.lineCount(), ch: j.getTextArea().value.length}), w.data("cmEditor", j)
						}
					} else c.bCodeMirror && (j = w.data("cmEditor"), w.val(j.getValue()), j.toTextArea()), v.html(w.val() || f.emptyPara), v.height(x.height ? w.height() : "auto"), e.activate(r), v.focus();
					e.updateCodeview(p.toolbar(), F)
				}
				u(g)
			}
		}, z = 24, A = function (b) {
			var c = a(document), d = l(b.target).editable(), e = d.offset().top - c.scrollTop();
			...
			var j = {c: Math.ceil(c.x / B) || 1, r: Math.ceil(c.y / B) || 1};
			g.css({
				width: j.c + "em",
				height: j.r + "em"
		...
		this.bindKeyMap = function (a, b) {
			...
			var e = b.keyMap[c.bMac ? "mac" : "pc"];
			this.bindKeyMap(a, e), a.editable.on("mousedown", t), a.editable.on("keyup mouseup", u), a.editable.on("scroll", v), b.disableDragAndDrop || D(a), a.handle.on("mousedown", w), a.toolbar.on("click", y), a.popover.on("click", y), a.toolbar.on("mousedown", x), a.popover.on("mousedown", x), a.statusbar.on("mousedown", A);
			var f = a.toolbar, g = f.find(".note-dimension-picker-mousecatcher");
			g.on("mousemove", C), a.editable.on("blur", function () {
			...
			}), b.onfocus && a.editable.focus(b.onfocus), b.onblur && a.editable.blur(b.onblur), b.onkeyup && a.editable.keyup(b.onkeyup), b.onkeydown && a.editable.keydown(b.onkeydown), b.onpaste && a.editable.on("paste", b.onpaste), b.onToolbarClick && a.toolbar.click(b.onToolbarClick), a.editable.data("callbacks", {
				onChange: b.onChange,
		...
		b = {
			picture: function (a) {
				...
				var b = ["Serif", "Sans", "Arial", "Arial Black", "Courier", "Courier New", "Comic Sans MS",
					"Helvetica", "Impact", "Lucida Grande", "Lucida Sans", "Tahoma", "Times", "Times New Roman",
					"Verdana"], c = '<button type="button" class="btn btn-default btn-sm btn-small dropdown-toggle" data-toggle="dropdown" title="' + a.font.name + '" tabindex="-1"><span class="note-current-fontname">Arial</span> <b class="caret"></b></button>';
		...
		var p = function (b, d) {
		...
		}, q = [["#000000", "#424242", "#636363", "#9C9C94", "#CEC6CE", "#EFEFEF", "#F7F7F7", "#FFFFFF"],
			...
			["#630000", "#7B3900", "#846300", "#295218", "#083139", "#003163", "#21104A",
				"#4A1031"]], r = function (b) {
			b.find(".note-color-palette").each(function () {
				for (var b = a(this), c = b.attr("data-target-event"), d = [], e = 0, f = q.length; f > e; e++) {
					for (var g = q[e], h = [], i = 0, j = g.length; j > i; i++) {
						var k = g[i];
						h.push(['<button type="button" class="note-color-btn" style="background-color:', k,
							';" data-event="', c, '" data-value="', k, '" title="', k,
